...
@interface CSRemotePlayer () <NSCopying, NSMutableCopying>
{
    // 标识用户进行了手动暂停
    BOOL _isUserPause;
}
...
/**
 资源加载代理
 */
@property (nonatomic, strong) CSRemoteResourceLoaderDelegate *resourceLoaderDelegate;

@end

@implementation CSRemotePlayer
...
- (void)playWithURL:(NSURL *)url isCache:(BOOL)isCache {
    ...
    self.resourceLoaderDelegate = [CSRemoteResourceLoaderDelegate new];
...
- (void)pause {
    ...
    _isUserPause = YES;
    if (self.player) {
        self.state = CSRemotePlayerStatePause;
    }
}
...
- (void)resume {
    ...
    if (self.player && self.player.currentItem.playbackLikelyToKeepUp) {
        self.state = CSRemotePlayerStatePlaying;
    }
}
...
- (void)stop {
    ...
    self.player = nil;
    if (self.player) {
        self.state = CSRemotePlayerStateStopped;
    }
}
...
/**
 ...
 @param state 状态
 */
- (void)setState:(CSRemotePlayerState)state {
    _state = state;
...
-(void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context {
    ...
    if ([keyPath isEqualToString:@"status"]) {
        ...
        if (status == AVPlayerItemStatusReadyToPlay) {
            ...
            NSLog(@"状态未知");
            self.state = CSRemotePlayerStateFailed;
        }
        ...
        if (ptk) {
            ...
            self.state = CSRemotePlayerStateLoading;
        }
...
#pragma mark - 单例方法
static CSRemotePlayer *_shareInstance;
+ (instancetype)shareInstance {
    if (!_shareInstance) {
        _shareInstance = [[CSRemotePlayer alloc] init];
    }
